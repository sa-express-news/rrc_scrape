...
class RRCCounty
	...
	def write_leases_page(html)
		File.open("#{@county}/#{@current_page}.html", 'w') { |f| f.write(html) } 
		@current_page += 1
	end

	def write_ptd_page(html, lease_num)
		File.open("#{@county}/leases/ptd/ptd_#{lease_num}.html", 'w') { |f| f.write(html) } 
	end

	def write_disp_page(html, lease_num)
		File.open("#{@county}/leases/disp/disp_#{lease_num}.html", 'w') { |f| f.write(html) } 
	end

	def write_cnty_pd_page(html, lease_num)
		File.open("#{@county}/leases/cnty_pd/cnty_pd_#{lease_num}.html", 'w') { |f| f.write(html) } 
	end
